...
public class EventsReplayController {
    ...
    public ResponseEntity<String> replayEvents(@PathVariable String processorName) {
        ...
        if (trackingEventProcessor.isPresent()) {
            ...
            return ResponseEntity.ok().body(String.format("the event processor with name [%s] has been reset",
                    processorName)
            );
        } else {
            return ResponseEntity.badRequest().body(String.format("the event processor with name [%s] is not" +
